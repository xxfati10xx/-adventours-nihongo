    ...
    <section className={`p-4 md:p-8 rounded-[2rem] border-2 flex flex-col h-[70vh] animate-fade-in-up ${isDarkMode ? 'bg-[#1A1A1A] border-[#2D2D2D] shadow-2xl' : 'bg-white border-[#E8DCC4] shadow-lg'}`}>
      ...
      <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar space-y-2">
        {dictionary
          ...
          .map((d, i) => (
            <div key={i} className={`flex justify-between items-center p-3 rounded-xl border-2 transition-all shadow-sm group ${isDarkMode ? 'bg-[#121212] border-[#2D2D2D] hover:border-[#D4AF37]' : 'bg-white border-[#FAF7F2] hover:border-[#BC2424]'}`}>
